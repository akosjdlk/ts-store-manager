...
type ModalInputType = "table" | "text" | "number" | "select";
...
type ModalInputConfig = TableInput | TextInput | SelectInput;
...
export class CustomModal {
	...
	private inputs: ModalInputConfig[];
	private footerInput: TextInput | null;
			...
			<div class="relative p-4 w-full max-w-4xl max-h-full">
				<div class="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
					<div class="flex items-center justify-between border-b border-default pb-4 md:pb-5">
						...
						<button type="button" class="custom-modal-close text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center">
					...
					<div class="custom-modal-body py-4 md:py-6 relative overflow-x-auto shadow-xs ms-4 mt-4 me-4"></div>

					<div class="flex items-center space-x-4 border-t border-default pt-4 md:pt-6">
						<button type="button" class="custom-modal-submit inline-flex items-center text-white bg-brand hover:bg-brand-strong box-border border border-transparent focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">
						...
						<div class="custom-modal-footer-inputs flex items-center space-x-4"></div>

						<button type="button" class="!ml-auto custom-modal-close text-body bg-neutral-secondary-medium box-border border border-default-medium hover:bg-neutral-tertiary-medium hover:text-heading focus:ring-4 focus:ring-neutral-tertiary shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">
							${this.cancelText}
						</button>
					</div>
				</div>
	...
	private renderInputs(): void {
		const bodyContainer = this.modalEl.querySelector<HTMLDivElement>(".custom-modal-body")!;
		const footerInputsContainer = this.modalEl.querySelector<HTMLDivElement>(".custom-modal-footer-inputs")!;

		this.inputs.forEach((config) => {
			switch (config.type) {
				case "text":
					this.RenderTextInput(config, bodyContainer);
					break;
				case "number":
					this.RenderTextInput(config, bodyContainer);
					break;
				case "select":
					this.RenderSelectInput(config, bodyContainer);
					break;
				case "table":
					this.RenderTableInput(config, bodyContainer);
					break;
			}
		});
	...
	private RenderSelectInput(config: SelectInput, container: HTMLDivElement): void {
		const optionsHtml = config.options.map((opt) => `<option value="${opt.value}" ${config.selectedValue === opt.value ? "selected" : ""}>${opt.label}</option>`).join("");

		const div = document.createElement("div");
		div.className = "col-span-2";
		div.innerHTML =`
			<label for="${config.id}" class="block mb-2.5 text-sm font-medium text-heading">
				${config.label}
			</label>
			<select id="${config.id}" name="${config.id}" class="block w-full px-3 py-2.5 bg-neutral-secondary-medium border border-default-subtle text-heading text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-body">
				<option value="" disabled ${config.selectedValue === "" ? "selected" : ""}>Select...</option>
				${optionsHtml}
			</select>
		`;
		...
		this.trackedInputs.push({
			id: config.id,
			type: "select",
			getValue: () => select?.selectedIndex === 0 ? null: select?.value,
		})
	}

	private RenderTableInput(config: TableInput, container: HTMLDivElement): void {
		...
		table.addEventListener("click", (e: MouseEvent) => {
			...
			const footerInput = this.trackedInputs.find((i) => i.id === this.footerInput!.id);
			if (footerInput?.element) {
				footerInput.element.focus();
				footerInput.element.select();
				footerInput.element.classList.add("ring-4", "ring-brand-medium");

				setTimeout(() => {
					footerInput.element?.classList.remove("ring-4", "ring-brand-medium");
				}, 800);
			}
		});
		...
		container.appendChild(config.table.wrapperDOM);
		this.trackedInputs.push({
			id: config.id,
			type: "table",
	...
	private setupEvents(): void {
		...
		this.modalEl.querySelectorAll(".custom-modal-close").forEach((btn) => {
			btn.addEventListener("click", () => {
				this.destroy(true);
			});
		});

		const submitBtn = this.modalEl.querySelector(".custom-modal-submit");
...
export function confirmationModal(
	...
	popup.innerHTML = `<div class="relative p-4 w-full max-w-md max-h-full">
		<div class="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
	...
	popup
		.querySelectorAll<HTMLButtonElement>("button[data-result]")
		.forEach((btn) => {
			btn.addEventListener("click", () => {
