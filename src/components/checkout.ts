...
async function render(): Promise<void> {
    ...
    const t = new DataTable('#selection-table', { data: data,
        ...
        top: "hidden flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4",
        ...
        dropdown: "hidden sm:flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400",
        selector: "hidden bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg",
        
        container: "overflow-x-auto",
        
        bottom: "hidden flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4 border-t border-gray-200 dark:border-gray-700",
        info: "text-sm font-normal text-gray-400 text-gray-400",
        
        pagination: "hidden inline-flex items-stretch",
    }
    })
    t.init()
    console.log("asd")
